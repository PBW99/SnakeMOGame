...
void OutputMemoryBitStream::Write(pair<int, int> inData)
{
	Write(inData.first);
	Write(inData.second);
}

void OutputMemoryBitStream::Write(CONSOLE_COLOR inData)
{
	Write(as_integer(inData));
}
...
void InputMemoryBitStream::Read(pair<int, int>& outData)
{
	ReadBits(&(outData.first), 32);
	ReadBits(&(outData.second), 32);
}

void InputMemoryBitStream::Read(CONSOLE_COLOR& outData)
{
	ReadBits(&outData, 32);
}
