...
public:
	...
	void Write( char inData, uint32_t inBitCount = 8 )	{ WriteBits( &inData, inBitCount ); }
	*/
	...
	void Write(pair<int, int> inData);
	void Write(CONSOLE_COLOR inData);


	// Vector Template Write
	template <typename T>
	void Write(const std::vector<T>& inVector)
	{
		size_t elementCnt = inVector.size();
		Write(elementCnt);
...
//	template <>            	void Read(pair<int, int>& outData, uint32_t inBitCount){}
//}
...
public:
	...
	void Read(T& outData, uint32_t inBitCount = sizeof(T) * 8)
	{
		...
		void Read(char&      outData, uint32_t inBitCount = 8) { ReadBits(&outData, inBitCount); }
	...
	void Read(pair<int, int>& outData);

	void Read(CONSOLE_COLOR& outData);

	// Vector Template Read
	template <typename T>
	void Read(vector<T>& outVector)
	{
		size_t elementCnt;
		Read(elementCnt);
		outVector.resize(elementCnt);
