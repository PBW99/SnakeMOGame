...
//	mSocket->Bind( ownAddress );
	mSocket = SocketUtil::CreateTCPSocket( INET );
	SocketAddress ownAddress( INADDR_ANY, inPort );
	mSocket->Bind( ownAddress );
...
//		int readByteCount = mSocket->ReceiveFrom( packetMem, packetSize, fromAddress );
		int readByteCount = mSocket->Receive( packetMem, packetSize, fromAddress );
		if( readByteCount == 0 )
		{
			//nothing to read
			break;
		...
		{
		DP("------------------------read From Sock" << fromAddress.ToString())
			inputStream.ResetToCapacity( readByteCount );
			++receivedPackedCount;
			totalReadByteCount += readByteCount;

			//now, should we drop the packet?
			if( UtilMath::GetRandFloat() >= mDropPacketChance )
			{
				//we made it
...
{
	DP("-----SendTO Sock" << inFromAddress.ToString())

	int sentByteCount = mSocket->Send( inOutputStream.GetBufferPtr(), inOutputStream.GetByteLength(), inFromAddress );
	if( sentByteCount > 0 )
	{
