...
void NetworkManagerClient::HandleStatePacket( InputMemoryBitStream& inInputStream )
{
	if( mState == NCS_Welcomed )
	{
		...
		//TODO move this to adequate
		if (World::GetIns().mPlayer == nullptr)
		{
			for (const auto& go : mNetworkIdToGameObjectMap)
			{
				Snake* snake = go.second->GetAsSnake();
				if (snake && snake->mPlayerID == mPlayerId)
				{
					World::GetIns().mPlayer = std::dynamic_pointer_cast<Snake>(go.second);
				}
			}
...
void NetworkManagerClient::ReadLastMoveProcessedOnServerTimestamp( InputMemoryBitStream& inInputStream )
{
	...
	DP("Read TS Dirty" << isTimestampDirty)
	...
	if( isTimestampDirty )
	{
		...
		InputManager::GetIns().GetMoveList().RemovedProcessedMoves( mLastMoveProcessedByServerTimestamp );
...
void NetworkManagerClient::SendInputPacket()
{
	...
	MoveList& moveList = InputManager::GetIns().GetMoveList();
